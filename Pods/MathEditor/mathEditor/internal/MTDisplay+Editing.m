...
- (MTDisplay*) subAtomForIndex:(MTMathListIndex*) index
{
    ...
    if (index.subIndexType == kMTSubIndexTypeSuperscript || index.subIndexType == kMTSubIndexTypeSubscript) {
        for (int i = 0; i < [self.subDisplays count]; i++) { // EDITED
            MTDisplay *atom = [self.subDisplays objectAtIndex:i]; // ADDED
            ...
            // BEGIN EDITS
            else if ([atom isKindOfClass:[MTLargeOpLimitsDisplay class]] && i == index.atomIndex) {
                MTLargeOpLimitsDisplay *lineAtom = (MTLargeOpLimitsDisplay *)atom;
                if (index.subIndexType == kMTSubIndexTypeSuperscript) { // upper limit
                    return lineAtom.upperLimit;
                }
                else { // lower limit
                    return lineAtom.lowerLimit;
                }
            }
            // END EDITS
        }
